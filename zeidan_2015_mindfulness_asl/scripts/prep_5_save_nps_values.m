...
subjids = []
for i = 1:size(cimgs{5},1)
    subjids = [subjids; cimgs{1,5}{i}(115:122)];
end
...
nps_table = addvars(nps_table, subjids,'Before', 'BookPost') % add subject IDs as first column

save(fullfile(resultsdir,'npsvals_mindfulness_zeidan.mat'), 'nps_table') % save
