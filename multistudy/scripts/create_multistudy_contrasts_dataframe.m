...
for i=1:size(dataset_names, 1)
    ...
    contrast_names = contrasts.DATA_OBJ_CONsc{1}.image_names;
    [worthless, len] = size(contrast_names);
    this_data = [];
    for q = 1:len
        this_contrast = strcat(dataset_keys(i),'_', contrast_names{q});
        this_data = contrasts.DATA_OBJ_CONsc{q};
        [n, k] = size(this_data);
        ...
        multistudy_contrasts_data_scaled{end+1} = fmri_ds;
...
multistudy_contrasts_scaled.data = multistudy_contrasts_data_scaled;
multistudy_contrasts_scaled.contrast_names = multistudy_contrasts_names;

savename = strcat('multistudy_contrasts_scaled_', date, '.mat')
save(fullfile(results_dir, savename), 'multistudy_contrasts_scaled') % save
